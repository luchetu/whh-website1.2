  ...
  return (
    <article className="grid h-full grid-rows-[auto_1fr_auto] rounded-2xl bg-[#0A0A0C1A] p-5 text-[#181818]">
      <div className="flex min-h-[4.5rem] items-start gap-3">
        ...
        <h3 className="max-w-[14rem] text-lg font-medium leading-snug tracking-tight">{title}</h3>
      </div>
      <p className="mt-5 max-w-[16rem] text-sm leading-6 text-[#4e4e4e]">{description}</p>
      <Link
        href="/services"
...
export function AboutServicesSection() {
  const cards = [...serviceItems, ...serviceItems]
